"""Tool registry for CyberSentinel agents."""
...
class ToolRegistry:
    ...
    def _register_core_tools(self) -> None:
        ...
        self.register_tool("query_clickhouse", self._query_clickhouse)
    ...
    # ClickHouse tools
    def _query_clickhouse(self, query: str, limit: int = 100) -> Dict[str, Any]:
        """Execute raw ClickHouse query."""
        if not self.clickhouse:
            return {"error": "ClickHouse client not available"}
        
        try:
            # In production, would use parameterized queries for safety
            result = self.clickhouse.query(query)
            return {
                "success": True,
                "rows": len(result.result_rows) if hasattr(result, 'result_rows') else 0,
                "data": result.result_rows[:limit] if hasattr(result, 'result_rows') else []
            }
    ...
    def _query_alerts(self, incident_id: str = None, 
        ...
        """Query alerts for incident correlation."""
        if not self.clickhouse:
            return {"error": "ClickHouse client not available"}
    ...
    def _query_attack_chain(self, start_technique: str, max_depth: int = 3) -> Dict[str, Any]:
        """Query for potential attack chains."""
        if not self.neo4j:
            return {"error": "Neo4j client not available"}
        
        try:
            # This would use the KnowledgeGraphManager in production
            cypher = """
            MATCH (start:TTP {technique_id: $start_technique})-[:PART_OF]->(tactic:Tactic)
            MATCH (tactic)<-[:PART_OF]-(related:TTP)
            WHERE start <> related
            ...
            with self.neo4j.driver.session() as session:
                result = session.run(cypher, start_technique=start_technique)
